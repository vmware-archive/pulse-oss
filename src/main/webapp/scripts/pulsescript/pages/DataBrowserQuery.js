...
function createMemberAccordionContentHtml(memberData){
  ...
  memberAccordionContentHtml += 
                "<div class=\"ScrollPaneBlock\" style=\"height: 358px;\">" +
                    ...
                    "<div class=\"n-listing\">" +
                      ...
                      "<div class=\"n-accordion-content\">" +
                        "<div class=\"widthfull-100per\">" +
                          "<div class=\"ScrollPaneBlock customeCloumnExtended\" style=\"height:"+ScrollPaneBlockHeight+"px; width:auto\">" +
...
function generateTableIdForObjectsTable(memberName, objectName){
  ...
  var objName = objectName.replace(/\.+/g, '');
...
function createResultGrid(member, memberResultObject){
  ...
  for(key in objectResults[0]){
    // add column and column model
    columnName.push(key);
...
function displayPopUpObjectExplorer(rowId, rowIndex, columnIndex, event){
  ...
  for(var i=0; i< memberResults.length; i++){
    if(memberResults[i].objectType == selMemObject){
    ...
    for(var i=0; i<rawResponeDataResult.length; i++){
      if(typeof(rawResponeDataResult[i]) == "object"){
        if($.isArray(rawResponeDataResult[i])){
          ...
          responseForMember.member = rawResponeDataResult[i].member;
          responseForMember.result = convertToExpectedObjectsFormat(rawResponeDataResult[i].result, rawResponeDataResult[i].member[1]);
      ...
      if(expResponseResult.length > 0){
        ...
        for(var j=0 ; j < expResponseResult.length ; j++){
          if(expResponseResult[j].objectType == rawResponseResult[i][0]){
            ...
            if(prefixForId != ""){
              entry.uid = prefixForId + "*" 
                          + expResponseResult[j].objectType 
                          + "*" + objectResults.length;
            }else{
              entry.uid = expResponseResult[j].objectType 
                          + "*" + objectResults.length;
            }
            
            objectResults.push(rawResponseResult[i][1]);
            break;
          }
        }
