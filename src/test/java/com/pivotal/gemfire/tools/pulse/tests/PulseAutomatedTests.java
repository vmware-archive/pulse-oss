...
class PulseBaseTests extends PulseTests {
  ...
  public HashMap<String, HashMap<String, Region>> getRegionDetailsFromUI(
      String regionName) {

    String[] regionNames = JMXProperties.getInstance().getProperty("regions")
        .split(" ");
  ...
  public void validateServerGroupGridData() {
    List<WebElement> serverGridRows = driver
        .findElements(By.xpath("//table[@id='memberListSG']/tbody/tr"));
    ...
    for (int j = 2, x = 0; j <= serverGridRows.size(); j++, x++) {
      for (int i = 0; i <= 6; i++) {
        gridDataFromUI[x][i] = driver
            .findElement(By.xpath("//table[@id='memberListSG']/tbody/tr[" + j
                + "]/td[" + (i + 1) + "]"))
            .getText();
      }
    }

    String[] memberNames = JMXProperties.getInstance().getProperty("members")
        .split(" ");
    HashMap<String, HashMap<String, Member>> sgMap =
        new HashMap<String, HashMap<String, Member>>();
  ...
  public void validateRedundancyZonesGridData() {
    List<WebElement> rzGridRows = driver
        .findElements(By.xpath("//table[@id='memberListRZ']/tbody/tr"));
    ...
    for (int j = 2, x = 0; j <= rzGridRows.size(); j++, x++) {
      for (int i = 0; i <= 6; i++) {
        gridDataFromUI[x][i] = driver
            .findElement(By.xpath("//table[@id='memberListRZ']/tbody/tr[" + j
                + "]/td[" + (i + 1) + "]"))
            .getText();
      }
    }
    ...
    String[] memberNames = JMXProperties.getInstance().getProperty("members")
        .split(" ");
    HashMap<String, HashMap<String, Member>> rzMap =
        new HashMap<String, HashMap<String, Member>>();
  ...
  public void validateTopologyGridData() {
    List<WebElement> rzGridRows = driver
        .findElements(By.xpath("//table[@id='memberList']/tbody/tr"));
    int rowsCount = rzGridRows.size();
    System.out.println(rowsCount);
    String[][] gridDataFromUI = new String[rowsCount][7];
    ...
    for (int j = 2, x = 0; j <= rzGridRows.size(); j++, x++) {
      for (int i = 0; i <= 6; i++) {
        gridDataFromUI[x][i] = driver
            .findElement(By.xpath("//table[@id='memberList']/tbody/tr[" + j
                + "]/td[" + (i + 1) + "]"))
            .getText();
      }
    }
  ...
  public void validateDataPrespectiveGridData() {
    List<WebElement> serverGridRows = driver
        .findElements(By.xpath("//table[@id='regionsList']/tbody/tr"));
    int rowsCount = serverGridRows.size();
    System.out.println(rowsCount);
    String[][] gridDataFromUI = new String[rowsCount][7];
    ...
    for (int j = 2, x = 0; j <= serverGridRows.size(); j++, x++) {
      for (int i = 0; i <= 6; i++) {
        if (i < 5) {
          gridDataFromUI[x][i] = driver
              .findElement(By.xpath("//table[@id='regionsList']/tbody/tr[" + j
                  + "]/td[" + (i + 1) + "]"))
              .getText();
        } else if (i == 5) {
          gridDataFromUI[x][i] = driver
    ...
    for (int i = 0; i < gridDataFromUI.length - 1; i++) {
      ...
      Assert.assertEquals(String.valueOf(r.getSystemRegionEntryCount()),
          gridDataFromUI[i][2]);
  ...
  public void navigateToToplogyView() {
    clickElementUsingXpath(
        PulseTestLocators.TopNavigation.clusterViewLinkXpath);
  ...
  public void navigateToDataPrespectiveGridView() {
    clickElementUsingXpath(
        PulseTestLocators.TopNavigation.clusterViewLinkXpath);
    clickElementUsingXpath(
        PulseTestLocators.DataPerspectiveView.downarrowButtonXpath);
    clickElementUsingXpath(
        PulseTestLocators.DataPerspectiveView.dataViewButtonXpath);
  ...
  public void navigateToRegionDetailsView() {
    ...
    clickElementUsingXpath(
        PulseTestLocators.RegionDetailsView.treeMapCanvasXpath);
  }

  public void navigateToRegionDetailsGridView() {
    navigateToRegionDetailsView();
    clickElementUsingXpath(
        PulseTestLocators.RegionDetailsView.gridButtonXpath);
  }

  public String getPropertyValue(String propertyKey) {
    String propertyValue = JMXProperties.getInstance()
        .getProperty(propertyKey);
    return propertyValue;
  }

  public void verifyElementPresentById(String id) {
    WebDriverWait wait = (WebDriverWait)new WebDriverWait(driver, maxWaitTime,
        500);
  ...
  public void verifyElementPresentByLinkText(String lnkText) {
    ...
    wait.until(ExpectedConditions
        .presenceOfAllElementsLocatedBy(By.linkText(lnkText)));
  }
  ...
  public void verifyTextPresrntById(String id, String text) {
    WebDriverWait wait = (WebDriverWait)new WebDriverWait(driver, maxWaitTime,
        500);
    wait.until(
        ExpectedConditions.textToBePresentInElementLocated(By.id(id), text));
  }
  ...
  public void assertMemberSortingByCpuUsage() {
    Map<Float, String> memberMap = new TreeMap<Float, String>(
        Collections.reverseOrder());
    String[] membersNames = splitString(
        JMXProperties.getInstance().getProperty("members"), " ");
    ...
    for (Map.Entry<Float, String> entry : memberMap.entrySet()) {
      // here matching painting style to validation that the members are painted
      // according to their cpu usage
      ...
      Assert.assertTrue(findElementById(entry.getValue())
          .getAttribute("style").contains(refMemberCPUUsage));
    }
  }

  public void assertMemberSortingByHeapUsage() {
    Map<Long, String> memberMap = new TreeMap<Long, String>(
        Collections.reverseOrder());
    String[] membersNames = splitString(
        JMXProperties.getInstance().getProperty("members"), " ");
    for (String member : membersNames) {
  ...
  public void assertMemeberSortingBySgHeapUsage() {
    ...
    for (Map.Entry<Float, String> entry : memberMap.entrySet()) {
      ...
      Assert.assertTrue(findElementById("SG1(!)" + entry.getValue())
          .getAttribute("style").contains(refMemberCPUUsage));
    }
  }
  ...
  public void assertMemeberSortingByRzHeapUsage() {
    ...
    for (Map.Entry<Float, String> entry : memberMap.entrySet()) {
      ...
      Assert.assertTrue(findElementById("RZ1 RZ2(!)" + entry.getValue())
          .getAttribute("style").contains(refMemberCPUUsage));
    }
  }
...
public class PulseAutomatedTests extends PulseBaseTests {
  ...
  public void regionName() {
    navigateToRegionDetailsView();
    Assert.assertEquals(getPropertyValue("region.R2.name"),
        getTextUsingId(PulseTestLocators.RegionDetailsView.regionNameDivId));
  }
  ...
  public void regionEmptyNodes() {
    ...
    Assert.assertEquals(getPropertyValue("region.R2.emptyNodes"),
        getTextUsingId(
            PulseTestLocators.RegionDetailsView.regionEmptyNodesId));
  }
  ...
  public void regionEntryCount() {
    ...
    Assert.assertEquals(getPropertyValue("region.R2.systemRegionEntryCount"),
        getTextUsingId(
            PulseTestLocators.RegionDetailsView.regionEntryCountTextId));
  }
  ...
  public void regionDiskUsage() {
    ...
    Assert.assertEquals(getPropertyValue("region.R2.diskUsage"),
        getTextUsingId(
            PulseTestLocators.RegionDetailsView.regionDiskUsageId));
  }
  ...
  public void regionPersistence() {
    navigateToRegionDetailsView();
    Assert.assertEquals(
        getPersistanceEnabled(
            getPropertyValue("region.R2.persistentEnabled")),
        getTextUsingId(
            PulseTestLocators.RegionDetailsView.regionPersistenceId));
  }
  ...
  public void verifyHostTooltipsOfTopologyGraphTest() {
    for (int i = 1; i <= 3; i++) {
      ...
      verifyTextPresrntByXpath(PulseTestLocators.TopologyView.cpuUsageTTXpath,
          "0%");
      verifyTextPresrntByXpath(
          PulseTestLocators.TopologyView.memoryUsageTTXpath,
          getPropertyValue("member.M" + i + ".currentHeapSize"));
      verifyTextPresrntByXpath(PulseTestLocators.TopologyView.loadAvgTTXpath,
          getPropertyValue("member.M" + i + ".loadAverage"));
  ...
  public void verifyMemberTooltipsOfTopologyGraphTest() {
    ...
    clickElementUsingId(PulseTestLocators.TopologyView.nodeH1Id);
    mouseClickAndHoldOverElementById(
        PulseTestLocators.TopologyView.memberM1Id);
    ...
    clickElementUsingId(PulseTestLocators.TopologyView.nodeH2Id);
    mouseClickAndHoldOverElementById(
        PulseTestLocators.TopologyView.memberM2Id);
    verifyTextPresrntByXpath(PulseTestLocators.TopologyView.memNameTTXpath,
        getPropertyValue("member.M2.member"));
    verifyTextPresrntByXpath(
        PulseTestLocators.TopologyView.memCpuUsageTTXpath,
    ...
    clickElementUsingId(PulseTestLocators.TopologyView.nodeH3Id);
    mouseClickAndHoldOverElementById(
        PulseTestLocators.TopologyView.memberM3Id);
    verifyTextPresrntByXpath(PulseTestLocators.TopologyView.memNameTTXpath,
  ...
  public void verifyMembersPresentInGridTest() {
    ...
    clickElementUsingId(PulseTestLocators.TopologyView.gridButtonId);
    verifyElementPresentByXpath(PulseTestLocators.TopologyView.nameM1Xpath);
    verifyTextPresrntByXpath(PulseTestLocators.TopologyView.nameM1Xpath,
        "M1");

    verifyElementPresentByXpath(PulseTestLocators.TopologyView.nameM2Xpath);
    verifyTextPresrntByXpath(PulseTestLocators.TopologyView.nameM2Xpath,
        "M2");

    verifyElementPresentByXpath(PulseTestLocators.TopologyView.nameM3Xpath);
    verifyTextPresrntByXpath(PulseTestLocators.TopologyView.nameM3Xpath,
        "M3");
  }
  ...
  public void verifyHostNamesInGridTest() {
    ...
    clickElementUsingId(PulseTestLocators.TopologyView.gridButtonId);
    verifyElementPresentByXpath(PulseTestLocators.TopologyView.hostH1Xpath);
    verifyTextPresrntByXpath(PulseTestLocators.TopologyView.hostH1Xpath,
        "h1");
    verifyElementPresentByXpath(PulseTestLocators.TopologyView.hostH2Xpath);
    verifyTextPresrntByXpath(PulseTestLocators.TopologyView.hostH2Xpath,
        "h2");
    verifyElementPresentByXpath(PulseTestLocators.TopologyView.hostH3Xpath);
    verifyTextPresrntByXpath(PulseTestLocators.TopologyView.hostH3Xpath,
        "h3");
  }
  ...
  public void verifyMembersPresentInSvrGrpTest() {
    ...
    clickElementUsingXpath(
        PulseTestLocators.ServerGroups.serverGrpsRadioXpath);
  ...
  public void expandAndCloseRdncyZoneTest() {
    ...
    verifyElementAttributeById(PulseTestLocators.RedundancyZone.zoneRZ1Id,
        "style", "width: 720px; height: 415px;");
    clickElementUsingXpath(PulseTestLocators.RedundancyZone.zoneRZ1RZ2Xpath);
    clickElementUsingXpath(PulseTestLocators.RedundancyZone.zoneRZ2Xpath);
    verifyElementAttributeById(PulseTestLocators.RedundancyZone.zoneRZ2Id,
        "style", "width: 720px; height: 415px;");
  ...
  public void clickRZGridShowingGridTest() {
    ...
    verifyTextPresrntByXpath(PulseTestLocators.RedundancyZone.idM2Xpath,
        "M2");
    verifyTextPresrntByXpath(PulseTestLocators.RedundancyZone.idM1Xpath,
        "M1");
    verifyTextPresrntByXpath(PulseTestLocators.RedundancyZone.idM3Xpath,
        "M3");
  }
