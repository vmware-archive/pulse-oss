...
import com.pivotal.gemfire.tools.pulse.internal.data.Cluster.Alert;
import com.pivotal.gemfire.tools.pulse.internal.data.Cluster.Client;
import com.pivotal.gemfire.tools.pulse.internal.data.Cluster.GatewayReceiver;
import com.pivotal.gemfire.tools.pulse.internal.data.Cluster.GatewaySender;
import com.pivotal.gemfire.tools.pulse.internal.data.Cluster.Member;
import com.pivotal.gemfire.tools.pulse.internal.data.Cluster.Region;
import com.pivotal.gemfire.tools.pulse.internal.data.IClusterUpdater;
...
public class PropMockDataUpdater implements IClusterUpdater {
  ...
  public boolean updateData() {
    ...
    Collection<Cluster.Region> regionsList = cluster.getClusterRegions().values();
    ...
    if (membersHMap.size() == 0) {
      ...
      for(Entry<String, Member> memberSet : membersHMap.entrySet())
      {
        ...
        List<com.pivotal.gemfire.tools.pulse.testbed.GemFireDistributedSystem.Region> thisMemberRegions = testbed.getRootDs().getRegions(memberSet.getValue().getName());

        int regionExists = 0;
        int index=0;
        for (com.pivotal.gemfire.tools.pulse.testbed.GemFireDistributedSystem.Region thisMemberRegion : thisMemberRegions) {
          ...
          if (regionsList.size() > 0) {
            ...
            if (regionExists == 0) {
              cluster.addClusterRegion(region.getFullPath(), region);
              regionsList = cluster.getClusterRegions().values();
            }
          } else {
            cluster.addClusterRegion(region.getFullPath(), region);
            regionsList = cluster.getClusterRegions().values();
  ...
  private Member initializeMember(String id, String name, boolean manager,
    ...
    if (gatewayReceiver == null) {
      gatewayReceiver = new Cluster.GatewayReceiver();
      m.setGatewayReceiver(gatewayReceiver);
    }
