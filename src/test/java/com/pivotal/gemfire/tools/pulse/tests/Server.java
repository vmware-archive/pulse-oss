...
public class Server {
  ...
  public Server(int port, String properties) throws IOException {
    ...
    url = new JMXServiceURL(formJMXServiceURLString("localhost", "" + port));
  ...
  private void addRegionMBean(String reg)
    ...
    for (String member : regionObject.getMembers()) {
      System.out.println("Adding Member MBean for Region:"+reg+" :: Member:"+member);
      mbs.registerMBean(regionObject, new ObjectName(
          "GemFire:service=Region,name=/"+reg+",type=Member,member=" + member));
    }
  }
